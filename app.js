var mobiles = {
  ...
  searchBtn.onclick = function () {
    ...
    if (b === "Select Brand" || m === "Select Model") {
        result.style.display = "block";
        result.innerHTML = "<h3>Please select brand and model</h3>";
        return;
    }

    // Get the model data
    let data = mobiles[b][m];
    ...
    result.innerHTML = `
        ...
        <p><b>RAM:</b> ${data.memory.ram}</p>
        <p><b>Storage:</b> ${data.memory.storage}</p>
        <p><b>Battery:</b> ${data.battery}</p>
        <p><b>OS:</b> ${data.operatingSystem}</p>
    `;
};
